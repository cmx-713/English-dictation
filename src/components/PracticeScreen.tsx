...
import { Play, Pause, CheckCircle, Eye, EyeOff, MessageSquare, Mic, ArrowLeft, Sparkles, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <div className={`max-w-4xl mx-auto pb-20 px-4 transition-all duration-300 ease-in-out ${isAiOpen ? 'mr-[400px]' : 'mx-auto'}`}>
        ...
        {/* 原文显示 */}
        {showOriginalText && (
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-xl p-6 mb-6 shadow-lg mt-4">
      ...
      */}
      <button
        ...
        className={`fixed bottom-8 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 z-[60] group border-4 border-white/20 ${
            isAiOpen 
            ? 'right-[400px] bg-slate-700 text-white hover:bg-slate-800' // 打开状态：灰色收起按钮
            : 'right-8 bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:scale-110' // 关闭状态：炫彩打开按钮
        }`}
      ...
      >
        {isAiOpen ? (
            ...
            <>
                ...
                <Sparkles size={16} className="absolute top-3 right-3 text-yellow-300 animate-pulse z-20" />
